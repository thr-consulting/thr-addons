...
export const WithTForm = () => {
	const [value, setValue] = useState<{text: string; masked: string}>();
	d(value);
	return (
		<Container>
			<TForm<FormValidationType> initialValues={{text: '', masked: ''}} validationSchema={formValidation} onSubmit={val => setValue({...val})}>
				{(props: TFormChildrenProps<FormValidationType>) => {
					const {values, handleSubmit, handleChange, handleBlur, setFieldValue, hasErrors, hasWarnings, renderWarnings} = props;
						...
						<Form onSubmit={handleSubmit} warning={hasWarnings} error={hasErrors}>
							...
							<Form.Field width={6}>
								...
								<MaskedInput
									...
									mask={{mask: '999-999'}}
									value={values.text}
									onChange={val => setFieldValue('text', val)}
									onBlur={handleBlur}
								/>
							</Form.Field>
